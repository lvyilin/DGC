...
class DGC(object):
    ...
    def __init__(self, opts, tag):
        ...
        self.reconstructed = decoder(opts, noise=self.encoded, is_training=self.is_training)
        ...
        self.decoded = decoder(opts, noise=self.sample_noise, is_training=self.is_training)
    ...
    def get_encoded(self, opts, mu, sig):
        ...
        # z = mu + tf.multiply(eps, tf.exp(sig / 2.))
        return z
